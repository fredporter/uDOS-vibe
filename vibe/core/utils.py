...
def _fallback_vibe_logger() -> logging.Logger:
    target_logger = logging.getLogger("vibe")
...
logger: Any = (
    get_udos_logger("core", category="vibe", name="vibe")
    if get_udos_logger is not None
    else _fallback_vibe_logger()
)
...
LOG_DIR = _PathRef(Path("memory/logs/udos/vibe"))
LOG_FILE = _PathRef(LOG_DIR.path / "vibe.log")


class StructuredLogFormatter(logging.Formatter):
    """Backward-compatible single-line formatter for legacy file logging tests."""

    def format(self, record: logging.LogRecord) -> str:
        ts = datetime.fromtimestamp(record.created, UTC).isoformat()
        message = record.getMessage().replace("\n", "\\n").strip()
        line = f"{ts} {record.process} {record.thread} {record.levelname} {message}"
...
def apply_logging_config(_target_logger: logging.Logger | Any = None) -> None:
    """Backward-compatible file logging config for legacy tests and adapters."""
    global logger
    logger = (
        get_udos_logger("core", category="vibe", name="vibe")
        if get_udos_logger is not None
    ...
    target_logger = _target_logger or logger
    ...
    live_module = sys.modules.get("vibe.core.utils")
    log_dir_ref = getattr(live_module, "LOG_DIR", LOG_DIR)
    log_file_ref = getattr(live_module, "LOG_FILE", LOG_FILE)

    log_dir = Path(log_dir_ref.path)
    log_file = log_dir / Path(log_file_ref.path).name
