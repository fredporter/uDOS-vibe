...
class GitRepository:
    ...
    def _repo_or_raise(self) -> Repo:
        if self._repo is None:
            try:
                # Explicit workdirs should not silently bind to a parent repo.
                # Default cwd behavior keeps parent discovery for CLI ergonomics.
                self._repo = Repo(
                    self._workdir,
                    search_parent_directories=not self._explicit_workdir,
                )
